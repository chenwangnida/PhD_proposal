...
\textbf{Dynamic Web service composition approaches} are discussed in the third related research area, where a dynamic service composition environment is handled. On the one hand, existing dynamic Web service composition mainly works on many various techniques for efficiently re-select component services for changes in QoS and service failures, such as R-tree, decision tree learning and RL. These approaches do not allow the changes of service composition structure. Apart from that, the cost of initial planning is ignored and separated from the adaption of dynamic environment. On the other hand, EC-based approach have not been utilized in dynamic web service composition so far and can overcome these two limitations due to their nature, where the changes of composition structure is not allowed and avoiding re-planning cost through re-using known plans (composite services) obtained prior. 


The last related research area focuses on \textbf{semantic Web service composition approaches}, which explore a more complex and realistic service composition. Most approaches on semantic Web service composition support precondition and effects, where loop composition construct is not handled and combinatorial optimization problems is left alone. On the whole, none of existing works achieve semantic web service composition supporting precondition and effects for all the composition constructs while simultaneously  optimizing QoS and quality of semantic matchmaking.
