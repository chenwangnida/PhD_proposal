...
\subsection{An Overview of Evolutionary Computation Techniques}\label{ec}
...
In summary, EC-based approaches have been shown their promises in solving web service composition problems, which aim to optimize one quality aspect, either QoS or the number of services in both semi-automated and fully automated ways. None of existing EC-based approaches aim to optimize both QoS and quality of semantic matchmaking simultaneously in a fully automated way.
...
\subsubsection{Dynamic Web Service Composition Approaches for Handling Changes in QoS}\label{dynamicQoS}
...
\subsubsection{Dynamic Web Service Composition Approaches for Handling Unexpected Service Failure}\label{dynamicService}
...
The second related research area is \textbf{EMO-based service composition}, in which different quality criteria of QoS are simultaneously optimized and a high quality Pareto Front is produced. Existing works on EMO-based approaches focus on QoS-aware semantic service composition in a semi-automated way. Sometimes, SLA constraints are considered. However, none of these approaches take QoS and quality of semantic matchmaking simultaneously in a fully automated way. Apart from that, posteriori articulation techniques for EMO are very promising research areas. To copy with our service composition problem, more effective user preference models and useful modifications of the posteriori articulation techniques demand further investigation.

\textbf{Dynamic Web service composition} aims at effectively handling a dynamic service composition environment. On the one hand, existing dynamic service composition mainly works on various techniques to efficiently handle changes in QoS and service failures, such as R-tree, decision tree learning and RL. These approaches do not allow the changes of service composition structure. Apart from that, the cost of initial planning is ignored and separated from the adaption of dynamic environment. On the other hand, EC-based approaches have not been utilized in dynamic service composition so far and their natures allow the changes of composition structure and avoid re-planning cost through re-using known plans (composite services) obtained prior. 


The last related research area focuses on \textbf{semantic Web service composition}, which explores a more complex service composition. Most approaches on Web service composition support preconditions and effects. However, loop composition construct is not handled and combinatorial optimization problems is left aside. On the whole, none of existing works establish a general matchmaking mechanism for satisfactions on preconditions and effects, which can effectively support loop and choice constructs.